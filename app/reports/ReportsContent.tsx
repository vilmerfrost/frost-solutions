...
function buildProfitabilityKPIs(totals?: ProfitabilityApiResponse['totals']): KPI[] {
  ...
  return [
    { label: 'Intäkter', value: revenue, previous: 0, format: 'sek' },
    { label: 'Kostnader', value: totalCost, previous: 0, format: 'sek' },
    { label: 'Marginal', value: marginPercent, previous: 0, format: 'percent' },
  ]
}
...
export default function ReportsContent() {
  ...
  const fetchProfitability = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiFetch<{ success: boolean; data: ProfitabilityApiResponse }>(`/api/reports/profitability?${params}`)
      const data = res.data
      const rows = (data?.rows ?? []).map((row) => ({
        name: row[groupBy] ?? 'Okänd',
        revenue: row.revenue ?? 0,
        cost: row.total_cost ?? 0,
        margin: row.margin ?? 0,
      }))

      setProfitKPIs(buildProfitabilityKPIs(data?.totals))
      setProfitRows(rows)
  ...
  async function handleExport() {
    try {
      ...
      const csvText = await apiFetch<string>('/api/reports/export', {
        method: 'POST',
      ...
      a.href = url
      a.download = `rapport-${activeTab}-${rangeLabel}.csv`
      a.click()
